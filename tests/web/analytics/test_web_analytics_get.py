...
from src.web.webroutes.analytics import get_logs, get_services


# TODO: This test could be better.
@patch('webroutes.analytics.get_services')
@patch('webroutes.analytics.get_logs')
def test_web_analytics_get_success(mock_get_logs, mock_get_services, client, admin_token):
...
@patch('requests.get')
def test_dashboard_get_logs_success(mock, admin_token):
...
def test_dashboard_get_logs_exception(mock, admin_token):
...
@patch('requests.get')
def test_dashboard_get_services_success(mock, admin_token):
    mock.return_value.status_code = 200
    mock.return_value.json.return_value = []
...
@patch('requests.get', side_effect=Exception('mocked error'))
def test_dashboard_get_services_exception(mock, admin_token):
    with pytest.raises(Exception) as excinfo:
