...
def test_create_apikey_valid_apikey(admin_token):
    body = {"name": "pytest-success", "role": "Editor"}
    ...
    get_response = client.get(
        f"/apikeys/{put_response.json()['_id']}", headers={"Authorization": admin_token}
    )
...
def test_create_apikey_invalid_role(viewer_token):
    body = {"name": "failed-test", "role": "Editor"}
    response = client.put(
        "/apikeys/", headers={"Authorization": viewer_token}, json=body
    )
    assert response.status_code == 401
