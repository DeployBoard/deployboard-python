...
def test_delete_apikey_valid_apikey(admin_token, apikey):
    response = client.delete(f"/apikeys/{apikey}", headers={"Authorization": admin_token})
...
def test_delete_apikey_bad_token(apikey):
    response = client.delete(f"/apikeys/{apikey}", headers={"Authorization": "bad-token"})
    assert response.status_code == 401
    assert response.json() == {"detail": "Not authenticated"}


def test_delete_apikey_invalid_role_viewer(viewer_token, apikey):
    response = client.delete(f"/apikeys/{apikey}", headers={"Authorization": viewer_token})
    assert response.status_code == 401
    assert response.json() == {"detail": "Unauthorized"}


def test_delete_apikey_invalid_role_editor(editor_token, apikey):
    response = client.delete(f"/apikeys/{apikey}", headers={"Authorization": editor_token})
    assert response.status_code == 401
    assert response.json() == {"detail": "Unauthorized"}


def test_delete_apikey_missing_key(admin_token):
    response = client.delete("/apikeys/000000000000000000000000", headers={"Authorization": admin_token})
