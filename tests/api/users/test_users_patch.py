...
def test_patch_user_valid_user(admin_token, user):
    body = {
        "first_name": "pytest_user",
        "password": "secret"
    }
    response = client.patch(f"/users/{user}", headers={"Authorization": admin_token}, json=body)
...
def test_patch_user_bad_token(user):
    body = {}
    response = client.patch(f"/users/{user}", headers={"Authorization": "bad-token"}, json=body)
...
def test_patch_user_invalid_role(viewer_token, user):
    body = {}
    response = client.patch(f"/users/{user}", headers={"Authorization": viewer_token}, json=body)
    assert response.status_code == 401
...
def test_patch_user_missing_user(admin_token):
    ...
    response = client.patch("/users/000000000000000000000099", headers={"Authorization": admin_token}, json=body)
