...
def test_get_logs_filter_application(admin_token):
    ...
    response = client.get(f"/logs/{query_string}", headers={"Authorization": admin_token})
    assert response.status_code == 200
    assert type(response.json()) == list
...
def test_get_logs_filter_multiple(admin_token):
    query_string = "?application=Sample&service=Api&from_timestamp=0&to_timestamp=9999999999"
    response = client.get(f"/logs/{query_string}", headers={"Authorization": admin_token})
    assert response.status_code == 200
    assert type(response.json()) == list
    assert response.json()[0]['application'] == 'Sample'
