...
def test_create_service_valid(admin_token):
    body = {"application": "Test", "service": "Pytest"}
    ...
    get_response = client.get(
        f"/services/{put_response.json()['_id']}",
...
def test_create_service_invalid_role(viewer_token):
    body = {"application": "TestViewer", "service": "PytestViewer"}
    response = client.put(
        "/services/", headers={"Authorization": viewer_token}, json=body
    )
    assert response.status_code == 401
...
def test_create_service_exists(mock, admin_token):
    body = {"application": "TestExists", "service": "PytestExists"}
