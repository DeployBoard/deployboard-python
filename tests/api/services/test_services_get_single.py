...
def test_get_service_valid_admin(admin_token, service):
    response = client.get(f"/services/{service}", headers={"Authorization": admin_token})
    assert response.status_code == 200
    assert type(response.json()) == dict


def test_get_service_valid_editor(editor_token, service):
    response = client.get(f"/services/{service}", headers={"Authorization": editor_token})
    assert response.status_code == 200
    assert type(response.json()) == dict


def test_get_service_valid_viewer(viewer_token, service):
    response = client.get(f"/services/{service}", headers={"Authorization": viewer_token})
    assert response.status_code == 200
    assert type(response.json()) == dict


def test_get_service_bad_token(service):
    response = client.get(f"/services/{service}", headers={"Authorization": "bad-token"})
