...
from models.services import NewService, ServiceResponse
...
async def get_all_services(current_user: User = Depends(get_current_active_user)):
    ...
    #  - Does that functionality belong here,
    #    or should it be up to the caller to perform the sort?
    #  - They would also have to call environments to get the order,
    #    we could do that for them here to save time.
    for service in db.services.find({"account": current_user["account"]}).sort(
        [("application", 1), ("service", 1)]
    ):
...
async def get_one_service(
    _id: str, current_user: User = Depends(get_current_active_user)
):
    """
    ...
    # find_one() returns NoneType if not found,
    # so we need to catch that case and Raise exception.
    if service is None:
...
async def create_service(
    body: NewService, current_user: User = Depends(get_current_active_user)
):
    """
    ...
    if db.services.find_one(service_dict) is not None:
        ...
        raise HTTPException(
            status_code=400, detail="Service already exists in this account."
        )
...
#     body: Service, current_user: User = Depends(get_current_active_user)
# ):
#     """
...
#     # passing upsert which will create object if does not exist.
#     try:
