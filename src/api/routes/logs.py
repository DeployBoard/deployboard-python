...
router = APIRouter(
    prefix="/logs", tags=["Logs"], responses={404: {"description": "Not found"}}
)
...
async def get_all_logs(
    ...
    if sort not in [-1, 1]:
        raise HTTPException(
            status_code=400, detail=f"sort must be -1 or 1, provided {sort}"
        )
    # Instantiate empty list.
    logs = []
    # For each document returned from find()
    # sorted by timestamp in the direction provided.
...
# Currently we have intentionally decided to not include a PUT/POST/DELETE method for
# the logs endpoint. We feel that the logs collection should be an immutable trail of
# the history of your environment. Providing a method to manipulate the data would
# invalidate the quality of the data. Of course manual manipulation is possible via
# direct access to the database, but that requires a much higher level of awareness of
# your decision before you touch the data.
