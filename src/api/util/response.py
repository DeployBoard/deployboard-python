...
def check_response(current_user, response):
    ...
    """
    # TODO: Recursively check all keys anywhere in the response. Currently this only
    #  checks top level keys.
    # TODO: Move this to a middleware that catches all outgoing responses,
    #  and doesn't rely on the method to call it explicitly.
    # Check if the response data contains items from account that is different
    # than our requester account.
    if current_user["account"] != response["account"]:
        # Log for debugging
        logger.critical(
            f"accounts do not match: {current_user['account']} != {response['account']}"
        )
        # Raise exception
