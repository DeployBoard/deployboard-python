...
def users_get_single(_id):
    """
    Queries our users api endpoint for specific user _id then passes that data into the users template
    """
    # TODO: Implement this
    try:
        # Call our api endpoint.
...
def users_post():
    ...
    try:
        # Call our api endpoint.
        response = webapi(method, f'users/{extras}', token=session['token'], json=user_data)
        # Log response for debugging.
        logger.debug(f'response: {response}')
    except Exception as e:
        # Log exception.
