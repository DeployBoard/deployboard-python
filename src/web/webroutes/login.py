...
def login_post():
    ...
    data = {"username": attempted_username, "password": request.form["password"]}
    try:
    ...
    logger.info(
        f"token_decoded: {token_decoded}"
    )  # TODO: The id is in the sub, should it be email?

    # TODO: we should make a second api call here to get the user's info
    #  and store it in the session
    session[
        "token"
    ] = token  # TODO: we need to store the token in httponly cookie, not in session.
    session["logged_in"] = True
    session["user_id"] = token_decoded["sub"]
    session["exp"] = token_decoded[
        "exp"
    ]  # TODO: Possibly remove this when refresh token is in place
